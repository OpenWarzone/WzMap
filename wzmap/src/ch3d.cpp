...
void Ch3DCrossProduct(float* a, float* b, float* c) {
	c[0] = a[1]*b[2]-a[2]*b[1];
	c[1] = a[2]*b[0]-a[0]*b[2];
	c[2] = a[0]*b[1]-a[1]*b[0];
}

float Ch3DDotProduct(float* a, float* b) {
...
unsigned int ConvexityTest3D(float* v, float* v0, float* v1, float* v2) {
	...
	if (test > EPSILON) {
		...
		v3[1] = v2[1]-v0[1];
		v3[2] = v2[2]-v0[2];
		Ch3DCrossProduct(v3, v, v4);
		// if v5 . v1 > v5 . v0 (or v5 . v2) then it is convex planar
		if (Ch3DDotProduct(v4, v1) > Ch3DDotProduct(v4, v0)) {
			return PLANAR;
		} else {
			return CONCAVE;
		}
...
void BuildPlanar(float* p, unsigned int lenp, unsigned int** l_p, unsigned int* lenl_p, unsigned int* leni_p, int listtype) {
	...
	for (i = 0; i < lenp; i++) {
		...
		v[i*3+1] = p[i*3+1] - c[1];
		v[i*3+2] = p[i*3+2] - c[2];
		theta[i] = atan2(Ch3DDotProduct(u1, &v[i*3]),
			             Ch3DDotProduct(u0, &v[i*3]));
		s[i] = i;
		t[i] = CONVEX;
	}
...
void BuildHull(float* p, unsigned int lenp, unsigned int** l_p, unsigned int* lenl_p, unsigned int* leni_p, int listtype) {
	...
	// allocate everything
	visited = (int*)malloc(3 * lenp * sizeof(int));
	memset(visited, 0, 3 * lenp * sizeof(int));

	q = (unsigned int*)malloc(3 * lenp * sizeof(unsigned int));
	memset(q, 0, 3 * lenp * sizeof(unsigned int));

	v = (float*)malloc(3 * lenp * sizeof(float));
	memset(v, 0, 3 * lenp * sizeof(float));

	theta = (float*)malloc(3 * lenp * sizeof(float));
	memset(theta, 0, 3 * lenp * sizeof(float));

	s = (unsigned int*)malloc(3 * lenp * sizeof(unsigned int));
	memset(s, 0, 3 * lenp * sizeof(unsigned int));

	s2 = (unsigned int*)malloc(3 * lenp * sizeof(unsigned int));
	memset(s2, 0, 3 * lenp * sizeof(unsigned int));

	t = (unsigned int*)malloc(3 * lenp * sizeof(unsigned int));
	memset(t, 0, 3 * lenp * sizeof(unsigned int));

	n = (float*)malloc(3 * (/*2*/4 * lenp) * sizeof(float));
	memset(n, 0, 3 * (/*2*/4 * lenp) * sizeof(float));
			...
			while (i1 < lenp) {
				...
				if (visited[s[i1%lenp]] != VISITED && visited[s[i2%lenp]] != VISITED) {
					...
					if (t[s[i1%lenp]] == PLANAR || t[s[i2%lenp]] == PLANAR) {
						Ch3DCrossProduct(&v[s[i1%lenp]], &v[s[i1%lenp]], u0);
						UnitVector(u0);
						for (i = 0; i < lenn1; i++) {
