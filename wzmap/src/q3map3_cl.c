...
qboolean GetPlatform(cl_platform_id *platform){
	clerror = clGetPlatformIDs(PLATFORM_IDS_SIZE, platform, (cl_uint *)&num_clplatforms); 
...
qboolean LoadProgramSource(char *filename){
	...
	PushKernel(result, (unsigned int *)&kernel_length); 
...
void do_stuff(){  
	...
	awesome = MAGIC_SIZE; 
		...
		LARGE_INTEGER tps, tick, begin, stop, cputime;
		cl_float4 news[MAGIC_SIZE];
		...
		for(i = 0; i < MAGIC_SIZE; i++) {
			...
			news[i] = src_a_h[i] * src_b_h[i];
		}
		QueryPerformanceCounter(&stop); 
		cputime.QuadPart = stop.QuadPart - begin.QuadPart; 
		Sys_Printf("It took %f milliseconds to add 8192 floats together on the CPU.\n(8192 pairs of floats from arrays).\n", 
               (float)(((float)cputime.QuadPart / (float)tps.QuadPart) * 1000));  
#endif
}