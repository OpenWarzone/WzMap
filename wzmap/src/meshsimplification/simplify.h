...
namespace Simplify
{
	...
	void simplify_mesh(int target_count, double agressiveness/*=7*/, bool verbose=false)
	{
		...
		{
			//printLabelledProgress("Simplification", iteration, 99);
			...
			//double threshold = 0.0001*pow(double(iteration + 3)/100.0, agressiveness);
			//double threshold = 0.000000000001*(double(iteration+3)*agressiveness);

			// target number of triangles reached ? Then break
			if ((verbose) && (iteration%5==0)) {
				Sys_Printf("iteration %d - triangles %d threshold %f\n",iteration,triangle_count-deleted_triangles, (float)threshold);
			}
		...
		//printLabelledProgress("Simplification", 99, 99);
	...
	void simplify_mesh_lossless(bool verbose=false)
	{
		...
		{
			//printLabelledProgress("Simplification", iteration, 9999);
			...
			if (verbose) {
				Sys_Printf("lossless iteration %d\n", iteration);
			}
		...
		//printLabelledProgress("Simplification", 9999, 9999);
	...
	void write_obj(std::string fileName)
	{
		...
		if (!file)
		{
			Sys_Printf("write_obj: can't write data file \"%s\".\n", fileName.c_str());
			return;
		}

		fprintf(file, "#############################################################\n");
		fprintf(file, "#  Simplify method collision model - Generated by WzMap...\n\n");
		fprintf(file, "#############################################################\n");
