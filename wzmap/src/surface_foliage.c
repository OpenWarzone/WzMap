...
void Foliage( mapDrawSurface_t *src )
{
	...
	for( foliage = si->foliage; foliage != NULL; foliage = foliage->next )
	{
		...
		InsertModel( foliage->model, 0, 0, transform, 1.0, NULL, NULL, qfalse, NULL, qfalse, qfalse, qfalse, src->entityNum, src->mapEntityNum, src->castShadows, src->recvShadows, 0, src->lightmapScale, NULL, src->minlight, src->minvertexlight, src->ambient, src->colormod, 0, src->smoothNormals, src->vertTexProj, src->noAlphaFix, 0, src->skybox, NULL, NULL, NULL, NULL, qfalse, 999999.0f);
