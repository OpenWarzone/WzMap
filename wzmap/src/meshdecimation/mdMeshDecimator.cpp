...
namespace MeshDecimation
{
    ...
    void MeshDecimator::InitializeQEM()
    {
        ...
        for(size_t v = 0; v < m_nPoints; ++v)
        {
			printLabelledProgress("InitializeQEMpoints", v, m_nPoints);
		...
		printLabelledProgress("InitializeQEMpoints", m_nPoints, m_nPoints);
        ...
        {
			printLabelledProgress("InitializeQEMedges", e, m_edges.size());
		...
		printLabelledProgress("InitializeQEMedges", m_edges.size(), m_edges.size());
    }
    ...
    {    
        ...
        for(int e = 0; e < nE; ++e)
        {
            ...
            double progress = ((double)e / (double)nE) * 100.0;
            /*if (fabs(progress-progressOld) > ptgStep && m_callBack)
            {
                //sprintf(msg, "%3.2f %% \t \t \r", progress);
                //(*m_callBack)(msg);
				//printLabelledProgress("InitializePriorityQueue", progress, 100);
		...
		printLabelledProgress("InitializePriorityQueue", 100, 100);
    }
    ...
    bool MeshDecimator::EdgeCollapse(double & qem)
    {
        ...
        {
			int a, b;
            int idEdge = m_vertices[v1].m_edges[itE];
        ...
        for(size_t itV = 0; itV< incidentVertices.Size(); ++itV)
        {
            int idVertex = incidentVertices[itV];
            for(size_t itE = 0; itE < m_vertices[idVertex].m_edges.Size(); ++itE)
            {
				int a, b;
                int idEdge = m_vertices[idVertex].m_edges[itE];
                a = m_edges[idEdge].m_v1;
                b = m_edges[idEdge].m_v2;
                if ( a!=v1 && b!=v1)
                {
    ...
    bool MeshDecimator::Decimate(size_t targetNVertices, size_t targetNTriangles, double targetError)
    {
              ...
              (qem < targetError))
        {
            ...
            if (fabs(progress-progressOld) > ptgStep && m_callBack)
            {
				...
				printLabelledProgress("Simplification", progress, 100);
                //(*m_callBack)(msg);
                progressOld = progress;
            }
			*/
